...
function fetchWeatherAlerts(lat, lon) {
  return fetch(`/api/weather?type=alerts&lat=${lat}&lon=${lon}`).then((r) =>
    r.json(),
  );
}
...
export function StatusRibbon({ activeModule }) {
  ...
  const contextMap = {
    escape: {
      tone: "critical",
      text: "Disaster Escape Assistant — Live hazard routing, flood warning detection, and active shelter guidance",
      icon: TriangleAlert,
    },
    route: {
      tone: "info",
      text: "Safe Route Planner — AI hazard avoidance routing bypassing flooded roads and landslides",
      icon: Navigation,
    },
    shelters: {
      tone: "safe",
      text: "Shelter Finder — Real-time shelter availability, capacity meters, and ADA/pet facilities",
      icon: Home,
    },
    family: {
      tone: "safe",
      text: "Family Locator — Track family member check-ins, shelter arrivals, and battery status",
      icon: Users,
    },
    profile: {
      tone: "info",
      text: "Profile & Settings — Configure your default location, family members, and app preferences",
      icon: Settings,
    },
    flood: {
      tone: "warning",
      text: "Smart Flood Monitoring — USGS river gauge sensors and satellite water boundary analysis",
      icon: Waves,
    },
    wildfire: {
      tone: "critical",
      text: "Wildfire Risk Tracker — NASA satellite thermal hotspots and wind direction vectors",
      icon: Flame,
    },
    earthquake: {
      tone: "info",
      text: "Earthquake Monitor — USGS seismic activity feed and epicenter proximity alerts",
      icon: Activity,
    },
    weather: {
      tone: "warning",
      text: "Weather Radar & Alerts — NWS extreme storm warnings and rainfall forecast maps",
      icon: CloudRain,
    },
  };
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        >
          <Menu size={18} color={C.red} strokeWidth={2.4} />
        </Button>
      ...
      >
        ...
        >
          <Compass
            size={14}
            color={loc.gpsStatus === "live" ? C.teal : C.textFaint}
          />
...
export function SideNav({
  ...
  const content = (
    <>
      ...
      {primaryModules.map((m) => {
        ...
        const toneColor =
          m.tag === "critical" ? C.red : m.tag === "warning" ? C.amber : C.teal;
        return (
          <Button
            key={m.id}
          ...
          >
            ...
            {showLabels && (
              <span style={{ whiteSpace: "nowrap" }}>{m.short}</span>
            )}
          </Button>
        );
      })}
      ...
      {monitoringModules.map((m) => {
        ...
        const isActive = active === m.id;
        const toneColor =
          m.tag === "critical" ? C.red : m.tag === "warning" ? C.amber : C.blue;
        return (
          <Button
            key={m.id}
            onClick={() => handleNav(m.id)}
          ...
          >
            ...
            >
              <Icon
                size={16}
                strokeWidth={2.2}
                color={isActive ? C.blue : C.textFaint}
                aria-hidden
              />
            </div>
            {showLabels && (
      ...
      >
        ...
        >
          <Settings
            size={16}
            strokeWidth={2.2}
            color={active === "profile" ? C.blue : C.textFaint}
            aria-hidden
          />
        </div>
  ...
  return (
    <nav
      ...
      style={{
        width: collapsed
          ? theme.TOKENS.sizes.leftNavCollapsed
          : theme.TOKENS.sizes.leftNav,
        flexShrink: 0,
        background: C.panel,
        borderRight: `1px solid ${C.line}`,
        display: "flex",
        ...
        gap: 6,
        transition:
          "width 0.2s ease, background-color 0.3s ease, border-color 0.3s ease, box-shadow 0.3s ease",
        boxShadow: S.sm,
      }}
    >
      {content}
...
export function AlertDrawer({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    const weatherReq = fetchWeatherAlerts(loc.lat, loc.lon)
      .then((d) => {
        if (d?.success)
          return (d.alerts || []).map((a) => ({ ...a, type: "weather" }));
        return [];
      })
      .catch(() => []);
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: C.panel2,
                  border: `1px solid ${a.type === "emergency" ? C.red + "44" : C.line}`,
                  borderRadius: 12,
                  padding: 16,
                }}
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  >
                    {a.headline || a.type || "Weather alert"}
                  </div>
                  {a.areaDesc && (
